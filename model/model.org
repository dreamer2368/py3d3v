...
by interpolating charge density to a uniform mesh and solving poissons
...
For example, the short range force between two particles using a
Gaussian screen function is calcuated exactly as
\begin{equation*}
  F_s(r_{ij}) =
  \begin{cases}
...
cut-off radius. This is shown in Figure \ref{fig:gaussian-screen}.


Care must be taken to avoid calculating all $\mathcal{O}(N^2)$
interactions between particles. To reduce the number of interations
calculated a second coarse mesh is defined that covers the
computational mesh, this is shown in Figure
\ref{fig:coarse-fine}. Each particle is assigned to one and only one
cell, defined as the region bounded by the 8 nodes closest to the
point, in the coarse mesh. For each particle short range interactions
are then calculated with the particles sharing its cell and those in
neighbooring cells. This reduces the interactions calculated to those
particles in the 27 cells nearest the particle.
...
From this it is simple to solve for $\hat{\phi}(\v{k})$ and recover
$\phi(\v{x})$ by using the inverse fourier transform.
...
$k_i \in \left\{ 2\frac{2\pi}{L_i}, 3\frac{2\pi}{L_i},
...
To calculate long range forces the charges are first intepolated
to the mesh using linear interpolation  weighting to find the mesh
charge density $\rho_m$. In the context of particle methods linear
interpolation is often refered to as Cloud-In-Cell (CIC) weighting. 
The modified charge density $\rho_l$ at grid point $\v{x}_p$ is
\[ \rho_l(\v{x}_p) = \sum_{p'} \rho_m(\v{x}_{p'}) G(\v{x}_p-\v{x}_{p'}) \]
where the summation is understood to be over mesh points $\v{x}_{p'}$ in
all periodic images of the domain.
This is a discrete convolution, so we can use the fourier
