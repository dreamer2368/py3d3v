...
computational mesh. Each particle belongs to one and only one cell,
defined as the region bounded by the 8 nodes closest to the point, in
the coarse mesh. For each particle short range interactions are then
calculated with the particles sharing its cell and those in
