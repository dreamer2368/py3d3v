...
\[ m \dot{v} = q E = -q \frac{d \phi}{d x}  \]
\[ -k^2 \hat{\phi} = \frac{q}{\epsilon_0} \hat{n} \]
\[ \implies -i \omega \hat{v} = i \frac{q^2}{\epsilon_0 k} \hat{n} \]
Apply the continuity equation to solve for $\hat{n}$ in terms of $\hat{v}$
\[ \pd{n}{t} + \pd{(n v)}{x} = 0  \]
where the second order terms are assumed small to get
