...
fixed radius $r_{\text{max}}$. The long range potentials are
evaluated by interpolating charge density to a uniform mesh and
solving poissons equation on the resulting mesh charge density. 

For simplicity, the current implementation of P3M reuses many of the
algorithms used in the PM method.

** Force calculation details
The current version of my P3M code is implemented using a Gaussian
screen function. This will be generalized shortly.

The screen function is defined as
\[ G(\v{r}; \beta) = \frac{\beta^3}{\pi^{3/2}} e^{-\beta^2 r^2} \]
and its fourier transform is
\begin{align*}
 \hat{G}(\v{k}; \beta) &= \int d^3v f(\v{r};\beta) e^{-i\v{k}\cdot\v{r}} \\
                   &= e^{-k^2/(2\beta)^2}
\end{align*}
...
\[ |\v{E}_G(\v{r})| = \frac{q}{2\epsilon_0\pi^{3/2}}\left[\frac{\sqrt{\pi}}{2r^2}erf(r\beta)
                                             -\frac{\beta}{r}e^{-\beta^2 r^2}\right] \]

The short range force between two particles is calcuated exactly as
\begin{equation*}
  |\v{F}_S(\v{r}_{ij})| =
  \begin{cases}
  ...
  \end{cases}
\end{equation*}
Where the parameter $r_{\text{max}}$ is the cut off radius. 

Care must be taken to avoid calculating all $\mathcal{O}(N^2)$ interactions
between particles. My code currently does not use any strategy to
reduce the number of interactions calcuated. This will be added soon.

*** Long Range Force Calculation (Particle-Mesh Force)
Charges are first intepolated to the mesh using Cloud-In-Cell (CIC) weighting
to find the mesh charge density $\rho_m$.

The modified charge density $\rho_l$ at grid point $\v{x}_p$ is
\[ \rho_l(\v{x}_p) = \sum_{p'} \rho_m(\v{x}_{p'}) G(\v{x}_p-\v{x}_{p'}) \]
where the summation is understood to be over all periodic images
of the domain. This is a discrete convolution, so we can use the fourier
convolution theorem to efficently calculate
\[ \hat{\rho}_l(\v{k}) = \hat{\rho}_m(\v{k}) \hat{G}(\v{k}) \]
The long-range potential is then easily calculated using the relation
\[ k^2 \hat{\phi_l}(\v{k}) = \frac{\hat{\rho}(\v{k})}{\epsilon_0} \]
An inverse FFT is then used to transform $\hat{\phi_l}(\v{k})$ to $\phi_l(\v{x}_p)$.
The electric field $\v{E}$ at each grid point is calculated using second order
centered finite differences, and interpolated to particle positions using
CIC interpolation.
...
7) Interpolate $\v{E}(\v{x}_p)$ at mesh points to $\v{E}(\v{x}_i)$ at
   particle positions using cloud in cell (CIC) interpolation
8) Advance particle velocities $\v{v}_i(t-\Delta t/2) \to \v{v}_i(t+\Delta t/2)$ using $\v{E}_i$
9) Advance particle positions $\v{x}_i(t) \to \v{x}_i(t+\Delta t)$ using $\v{v}_i(t+\Delta t/2)$
