...
\[ E_s(r) = \frac{q}{2\epsilon_0\pi^{3/2}}\left[\frac{\sqrt{\pi}}{2r^2}erf(r\beta)
...
The discrete fourier transform is the fourier transform for values
defined on a uniform mesh. It is defined as
...
where the summation is over all values of $\v{k}$ defined by the
...
to the mesh using Cloud-In-Cell (CIC) weighting to find the mesh
charge density $\rho_m$.
...
** HE P3M
I am in the first stages of implementing P3M 
From Hockney and Eastwood
\begin{equation*}
S2(r) =
\begin{cases}
...
By Gauss's Law we can calculate the field from an $S2$ particle $x_i$ as
\begin{equation*}
|E_i(r)| =
\begin{cases}
